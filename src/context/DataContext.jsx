...
export const DataProvider = ({ children }) => {
  const [selectedWeakDay, setWeakDay] = useState(
    JSON.parse(localStorage.getItem("selectedWeakDay")) || "Tuesday"
  )

  const [outfits, setOutfits] = useState(
    JSON.parse(localStorage.getItem("outfitList")) || [
      {
        id: 1,
  ...
  function handleOutfitCardClick(event) {
    ...
    const transformedOutfits = outfits.map((outfit) =>
      outfit.id === parseInt(event.currentTarget.id)
  ...
  return (
    <DataContext.Provider
      value={{
        outfits,
        selectedWeakDay,
        handleWeakDaySelectionChange,
        handleOutfitCardClick,
        setWeakDay,
      }}
    >
      {children}
