...
const GroupedWeakDays = () => {
  ...
  function groupWeakDaysOutfits() {
    ...
    var monday = {
      day: "Monday",
      options: mondayOutfit,
      collapsed: selectedWeakDay === "Monday" ? false : true,
    }
    days.push(monday)
    ...
    var tuesday = {
      day: "Tuesday",
      options: tuesdayOutfit,
      collapsed: selectedWeakDay === "Tuesday" ? false : true,
    }
    days.push(tuesday)

    var wednesdayOutfit = outfits.filter(
      (outfit) => outfit.weakDay === "Wednesday"
    )
    var wednesday = {
      day: "Wednesday",
      options: wednesdayOutfit,
      collapsed: selectedWeakDay === "Wednesday" ? false : true,
    }
    days.push(wednesday)
    ...
    var friday = {
      day: "Friday",
      options: fridayOutfit,
      collapsed: selectedWeakDay === "Friday" ? false : true,
    }
    days.push(friday)
  ...
  function handleDayClick(event) {
    var newOutfitData = groupedOutfits.map((groupedData) =>
      groupedData.day === event.currentTarget.id
                  ...
                  <div key={option.id} className='mt-2'>
                    <h5 className='card-title mt-2'>
                      <span className='text-dark'>
                        <b>Full Name:</b> {option.fullName}
                      </span>
                    </h5>
                    <p className='card-text text-dark mt-2'>
                      <b>Designation: </b> {option.designation}
                    </p>
                  </div>
                )
              })}
